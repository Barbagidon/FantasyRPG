...
2. **[PlayerTurnState.cs](../Assets/Scripts/Core/Combat/PlayerTurnState.cs):**
   - Состояние ожидания команды от игрока. `TrySpendAP` определён здесь, но на момент последнего аудита (2026-07-29) не вызывается ни из одного места — расход AP не подключён к выполнению команд. Это первая задача Вехи 0 в `docs/roadmap.md`, не решённая пока.
...
**Расхождение с кодом (найдено аудитом 2026-07-29, не исправлено):** этот файл раньше показывал `APCost` как поле интерфейса `ICombatCommand`. В реальном коде (`Assets/Scripts/Core/Combat/ICombatCommand.cs`) интерфейс объявляет только `CanExecute()` и `Execute()` — `APCost` не является частью контракта. Диаграмма выше исправлена под фактический код. Если `APCost` должен быть частью интерфейса (чтобы FSM могла проверять стоимость любой команды до вызова `Execute()`), это отдельное архитектурное решение, а не опечатка в доке — обсудить и внести в интерфейс осознанно, а не молча.

- **[AttackCommand.cs](../Assets/Scripts/Core/Combat/AttackCommand.cs):** Принимает атакующего героя и цель, проверяет `CanExecute()` (живы ли оба участника), должен списывать AP и вызывать расчёт через [DamageCalculator.cs](../Assets/Scripts/Core/Stats/DamageCalculator.cs). На момент аудита `Execute()` не тратит AP — см. пункт выше.
