...
1. **[InitState.cs](../Assets/Scripts/Core/Combat/InitState.cs):**
   - Инициализирует список участников боя, сортирует их по параметру `Initiative`. (Раньше здесь значился `Speed` — поле разделено на `Initiative` + `MoveSpeed` в Вехе 0, подтверждено `dotnet test` 2026-08-02.)
...
2. **[PlayerTurnState.cs](../Assets/Scripts/Core/Combat/PlayerTurnState.cs):**
   ...
   - ⚠️ **Расхождение с принятым дизайном:** `ResetActionPoints()` сбрасывает AP в начале хода, но [ADR-0002](./decisions/0002-action-points-and-movement-economy.md) фиксирует накопление — `min(Current + ActionPointsPerTurn, MaxActionPoints)`, неиспользованный остаток переносится. Это не опечатка в доке: код пока реализует старую модель, ADR принят, но не реализован. Правится вместе с переименованием `MoveSpeed` → `Movement` в Вехе 1 (см. `docs/roadmap.md`, пункт `MoveCommand`).
...
- **[AttackCommand.cs](../Assets/Scripts/Core/Combat/AttackCommand.cs):** Хранит `attackerId`/`targetId` (`int`), а не прямые ссылки на `Hero` — требование сетевого задела Вехи 0 (команды сериализуемы, см. `game_concept.md` §5). Резолвит юнитов через `TurnBasedCombatEngine.GetUnitById(int)` внутри приватного `Validate()`. Проверяет `CanExecute()` (живы ли оба участника — через `Hero.CurrentHealth`, а не устаревший путь `Hero.Stats.CurrentHealth`), списывает AP через `Hero.TrySpendAP(APCost)` и вызывает расчёт через [DamageCalculator.cs](../Assets/Scripts/Core/Stats/DamageCalculator.cs). Подтверждено `dotnet test` (2026-08-01) — `Execute()` реально тратит AP, это больше не расхождение.
...
- 🗺️ [Перемещение по сетке — текущий контракт](./movement_spec.md)
