...
- **⚠️ The `Pathfinder` contract spans three ADRs — ADR-0001 alone is stale.** ADRs are immutable, so later ones *append* to the contract instead of editing it. The current signature is `FindReachable(start, actionPointBudget, movement, mover, movementContext)` and the current predicates are `CanTraverse(from, to, mover)` / `CanStopAt(cell)` — both defined in **ADR-0003**, not ADR-0001. See `docs/decisions/README.md` for the per-concern "where is it current" table before designing against any of it.
